...
function startTime() {
    ...
    if(preTime===0){
        ...
        if (audio1.readyState >= 2) { // HAVE_CURRENT_DATA or higher
            audio1.play().catch(error => {
                console.error('Audio play failed:', error);
                // Try to continue without audio
                notifications.show('Audio playback failed, continuing without sound', 'warning');
            });
        } else {
            console.warn('Audio not ready, playing without sound');
            notifications.show('Audio not ready, playing without sound', 'warning');
        }
        preTime=-1;
    }
    if(audio1.ended){
...
function bt_start_onclick() {
    ...
    if (startDiv) startDiv.setAttribute("style","display: none;");
    
    const settingsContainer = document.getElementById("settingsContainer");
    if (settingsContainer) settingsContainer.style.display = "none";
    
    // Get scale value from the correct input
    let scaleInput = document.getElementById("inputScale");
    if (scaleInput) {
        Scale = Number(scaleInput.value);
    } else {
        Scale = 100; // Default value
    }
    
    // Setup canvas display
    document.getElementById("canvasdiv").setAttribute("style","width: "+String(600*Scale/100)+"px; margin:0 auto; display: block;");
    c.setAttribute("style","border:4px solid #7f7f7f; background: black; "+"width: "+String(600*Scale/100)+"px;");
    
    // Get other settings from inputs
    let scrollInput = document.getElementById("scrollDurationInput");
    if (scrollInput) {
        scrollDuration = Number(scrollInput.value);
    } else {
        scrollDuration = 450; // Default value
    }
    
    let offsetInput = document.getElementById("inputOffset");
    if (offsetInput) {
        offset = Number(offsetInput.value);
    } else {
        offset = -40; // Default value
    }
    
    // Handle base BPM
    let baseBPMInput = document.getElementById("baseBPM");
    if (baseBPMInput && baseBPMInput.value !== "") {
        baseMpB = 60000 / Number(baseBPMInput.value);
    } else {
